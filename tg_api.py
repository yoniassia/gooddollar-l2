#!/usr/bin/env python3
"""Paperclip API helper for Tester Gamma agent."""
...
# Per-issue run IDs (populated after checkout)
ISSUE_RUNS = {}
...
def checkout(issue_id):
    ...
    # Store the checkoutRunId for subsequent mutating requests
    run_id = result.get("checkoutRunId") or result.get("run_id") or RUN_ID
    ISSUE_RUNS[issue_id] = run_id
    return result

def patch(issue_id, body):
    return api("PATCH", "/issues/" + issue_id, body, run_id=ISSUE_RUNS.get(issue_id, RUN_ID))

def comment(issue_id, body_text):
    return api("POST", "/issues/" + issue_id + "/comments",
        {"body": body_text}, run_id=ISSUE_RUNS.get(issue_id, RUN_ID))
...
if __name__ == "__main__":
    ...
    if cmd == "info":
        ...
        print(json.dumps(checkout(sys.argv[2]), indent=2))
    elif cmd == "done":
        checkout(sys.argv[2])
        print(json.dumps(patch(sys.argv[2], {"status": "done", "comment": sys.argv[3]}), indent=2))
    elif cmd == "blocked":
        checkout(sys.argv[2])
        print(json.dumps(patch(sys.argv[2], {"status": "blocked", "comment": sys.argv[3]}), indent=2))
    elif cmd == "comment":
        checkout(sys.argv[2])
        print(json.dumps(comment(sys.argv[2], sys.argv[3]), indent=2))
    elif cmd == "patch":
        checkout(sys.argv[2])
        print(json.dumps(patch(sys.argv[2], json.loads(sys.argv[3])), indent=2))
