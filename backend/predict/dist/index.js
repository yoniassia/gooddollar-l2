...
import { PredictContractInteraction } from './contracts/ContractInteraction.js';
...
async function main() {
    ...
    const engine = new OrderBookEngine({
        onTrade: (trade) => {
            ...
            if (yesBook)
                wsServer?.broadcastOrderBook(yesBook);
            if (noBook)
                wsServer?.broadcastOrderBook(noBook);
            ...
            if (contracts) {
                ...
                if (onChainMarket && onChainMarket.onChainId >= 0) {
                    contracts.settleBuy(onChainMarket.onChainId, trade.token === 'YES', trade.size.toString(), trade.taker).then((result) => {
    ...
    if (contracts) {
        ...
        app.get('/api/v1/chain/balance/:address/:marketId/:token', async (req, res) => {
            try {
                ...
                const balance = await contracts.getTokenBalance(req.params.address, parseInt(req.params.marketId), isYES);
                res.json({
        ...
        app.post('/api/v1/chain/markets', async (req, res) => {
            try {
                ...
                const { txHash, marketId: onChainId } = await contracts.createMarket(question, endTimeSec, resolverAddr);
        ...
        app.post('/api/v1/chain/markets/:id/resolve', async (req, res) => {
            try {
                ...
                try {
                    await contracts.closeMarket(market.onChainId);
                }
