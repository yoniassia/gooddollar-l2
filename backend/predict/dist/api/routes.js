...
export function createRoutes(engine, feed, resolver) {
    const router = Router();
    // ============================================================
    // Markets
    // ============================================================
    ...
    router.get('/orderbook/:marketId/:token', (req, res) => {
        ...
        const book = engine.getOrderBook(marketId, token.toUpperCase());
        if (!book)
            return res.status(404).json({ error: 'Order book not found' });
        res.json(book);
    });
    /** Get midpoint price */
    router.get('/midpoint/:marketId/:token', (req, res) => {
        const { marketId, token } = req.params;
        const mid = engine.getMidpoint(marketId, token.toUpperCase());
    ...
    router.get('/orders/maker/:address', (req, res) => {
    ...
    router.get('/feeds/:marketId', (req, res) => {
        const priceFeed = feed.getFeed(req.params.marketId);
        if (!priceFeed)
            return res.status(404).json({ error: 'No price feed for this market' });
        res.json(priceFeed);
    });
    /** Get all price feeds */
    ...
    router.post('/feeds/:marketId/link', (req, res) => {
