#!/usr/bin/env python3
"""Checkout and work on GOO-185."""
import json, hmac, hashlib, base64, time, urllib.request, urllib.error, sys
...
if __name__ == "__main__":
    ...
    if cmd == "checkout":
        result = api("POST", "/issues/" + GOO_185_ID + "/checkout",
        ...
        result = api("PATCH", "/issues/" + GOO_185_ID, body)
        print(json.dumps(result, indent=2) if result else "None")
    elif cmd == "comment":
        result = api("POST", "/issues/" + GOO_185_ID + "/comments", {"body": sys.argv[2]})
        print(json.dumps(result, indent=2) if result else "None")
