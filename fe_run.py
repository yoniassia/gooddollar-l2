#!/usr/bin/env python3
"""Generic Paperclip API helper using a specific run ID."""
...
if __name__ == "__main__":
    ...
    cmd = sys.argv[2]
    if cmd == "done":
        print(json.dumps(api("PATCH", f"/issues/{sys.argv[3]}",
            {"status": "done", "comment": sys.argv[4]}, run_id=run_id), indent=2))
    elif cmd == "blocked":
        print(json.dumps(api("PATCH", f"/issues/{sys.argv[3]}",
            {"status": "blocked", "comment": sys.argv[4]}, run_id=run_id), indent=2))
    elif cmd == "comment":
        print(json.dumps(api("POST", f"/issues/{sys.argv[3]}/comments",
            {"body": sys.argv[4]}, run_id=run_id), indent=2))
    elif cmd == "patch":
        print(json.dumps(api("PATCH", f"/issues/{sys.argv[3]}",
            json.loads(sys.argv[4]), run_id=run_id), indent=2))
    elif cmd == "checkout":
        print(json.dumps(api("POST", f"/issues/{sys.argv[3]}/checkout",
