#!/usr/bin/env python3
"""Checkout helper that generates a fresh run ID per checkout."""
import sys, json, hmac, hashlib, base64, time, uuid, urllib.request, urllib.error
...
def api(method, path, body=None, run_id=None):
    ...
    try:
        ...
        return {"error": e.code, "body": e.read().decode()}, run_id

if __name__ == "__main__":
    issue_id = sys.argv[1]
    result, run_id = api("POST", f"/issues/{issue_id}/checkout",
        {"agentId": AGENT_ID, "expectedStatuses": ["todo","backlog","blocked","in_progress"]})
    print(json.dumps(result, indent=2))
    if "checkoutRunId" in result or result.get("status") == "in_progress":
        print(f"\nRUN_ID={result.get('checkoutRunId', run_id)}", file=sys.stderr)
