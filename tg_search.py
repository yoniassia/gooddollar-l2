...
data = api_get("/companies/" + COMPANY + "/issues?q=" + encoded)
issues = data if isinstance(data, list) else data.get("issues", [])
for i in issues:
    print(i.get("identifier", "?"), "|", i.get("status", "?"), "|", i.get("assigneeAgentId", "unassigned")[:8] if i.get("assigneeAgentId") else "unassigned", "|", i.get("title", ""))
